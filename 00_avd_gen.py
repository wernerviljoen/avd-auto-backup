...
def get_ip_pool():
    while True:
        try:
            ip_pool = ipaddress.IPv4Network(input("Enter the Management IP Pool in the format IP_address/Subnet_mask: "), strict=False)
            return ip_pool
        except ValueError:
...
if __name__ == "__main__":
    ...
    dc_number = get_numeric_input("How many DC’s have they got? ")
...
#    initial_topo_data['CVP_CONTAINERS_INIT']['STAGING']['configlets'].append(f'{cust_name}-INFRA-STAGING')    
    ...
    spines_count = get_numeric_input(f"How many SPINES for {dc_name}? ")
    leaf_pairs_count = get_numeric_input(f"How many LEAF pairs for {dc_name}? ")
